...
export async function sbSaveVolleyballMatch(match) {
  const row = mapMatchForDb({ id: match.id || uid(), ...match }, 'volleyball_matches');
  await sbUpsert('volleyball_matches', [row]);
}
...
// Score/status fields — update-only (added by supabase-schedule-upgrade.sql).
const MATCH_SCORE_FIELDS = ['home_score', 'away_score', 'status', 'duration_minutes'];
const VB_MATCH_SCHEMA_FIELDS = ['id', 'home_team', 'away_team', 'home_country', 'away_country', 'kickoff_time', 'status', 'home_score', 'away_score', 'round_label'];
