  ...
  return (
    <article className="news-article-page">
      ...
      <div className="news-article-images">
        {images.map((image, index) => <img key={`${image}-${index}`} src={image} alt="" />)}
        {!images.length && <div className="news-image-empty">FAGALLA NEWS</div>}
      </div>
        ...
        <>
          ...
          {featured && <button type="button" className="news-featured" onClick={() => setSelected(featured)}>
            <div className="news-featured-image">
              <img src={contentImage(featured)} alt="" />
            </div>
            ...
            <section className="news-feed">
              ...
              {remaining.map((article) => (
                <button type="button" key={article.id} onClick={() => setSelected(article)}>
                  ...
                  <span className="news-feed-image"><img src={contentImage(article)} alt="" /></span>
                </button>
              ))}
            </section>
          )}
          {!!ads.length && (
            <section className="news-sponsored">
              <h2>Sponsored</h2>
              <div>{ads.map((ad) => <a key={ad.id} href={ad.link_url || undefined} target={ad.link_url ? '_blank' : undefined} rel="noreferrer"><img src={contentImage(ad)} alt="" /><strong>{ad.title}</strong><span>AD</span></a>)}</div>
            </section>
          )}
        </>
      )}
